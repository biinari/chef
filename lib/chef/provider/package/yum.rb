    ...
    class Package
      class Yum < Chef::Provider::Package
        ...
        def check_resource_semantics!
          ...
          # If multipackage is enabled, throw an error if the number of packages, versions, and architectures do not match up.
          if invalid_package_version_specification?
            raise Chef::Exceptions::InvalidResourceSpecification, "Please make sure you specify a version and architecture for each package in the array. If no preference, use `nil`."
          end
        end
        ...
        def load_current_resource
          ...
          if new_resource.source
            ...
            # The following statements are a holdover of functionality that
            # _might_ not be the best idea. They effectively allow you to specify
            # a single version/architecture and have it apply to multiple packages
            # or the specify multiple versions/architecture and have it apply to
            # a single package.
            #
            # There's probably some dragons here, and this might be something we'll
            # want to deprecate in the future, but its the way that most people _think_
            # the resource should act and it isn't too hard to codify.
            check_resource_semantics_again = false

            # Install the same version of multiple packages
            if new_resource.package_name.is_a?(Array) && new_resource.version.is_a?(String)
              v = new_resource.version
              Chef::Log.info("#{new_resource} using version #{v} for all packages: #{package_name_array.join(" ")}")
              new_resource.version(package_name_array.map { v })
              check_resource_semantics_again = true
            end

            # Install the same architecture of multiple packages
            if new_resource.package_name.is_a?(Array) && new_resource.arch.is_a?(String)
              a = new_resource.arch
              Chef::Log.info("#{new_resource} using architecture #{a} for all packages: #{package_name_array.join(" ")}")
              new_resource.arch(package_name_array.map { a })
              check_resource_semantics_again = true
            end

            # Install multiple architectures of the same package
            if new_resource.package_name.is_a?(String) && new_resource.arch.is_a?(Array)
              n = new_resource.package_name
              Chef::Log.info("#{new_resource} installing package #{n} for the following architectures: #{safe_arch_array.join(" ")}")
              new_resource.package_name(safe_arch_array.map { n })
              check_resource_semantics_again = true
            end

            # Install multiple versions of the same pacakge
            if new_resource.package_name.is_a?(String) && new_resource.version.is_a?(Array)
              n = new_resource.package_name
              Chef::Log.info("#{new_resource} installing the following versions of package #{n}: #{new_version_array.join(" ")}")
              new_resource.package_name(new_version_array.map { n })
              check_resource_semantics_again = true
            end

            # We've done a lot of futzing with the resource in the above manipulations.
            # If we modified the resource, check it again for correctness.
            check_resource_semantics! if check_resource_semantics_again
        ...
        # Make sure that there are an equal number of package/version/architectures
        # specified in the resource. This check is performed at least two times:
        #   1) on the resource as the user has declared it
        #   2) on the resource as we have expanded it (see load_current_resource)
        def invalid_package_version_specification?
          num_pkgs = package_name_array.length
          return true if new_resource.version.is_a?(Array) && new_version_array.length != num_pkgs
          return true if new_resource.arch.is_a?(Array) && safe_arch_array.length != num_pkgs
